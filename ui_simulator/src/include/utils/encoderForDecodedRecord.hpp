 ...
 */
void encodeAndSetCatAndLength(unsigned int _categoryNumber, unsigned int _datablockSize,
                              vector<unsigned char> *encodedRecord) {
...
encodeAllItemsFromRecordCollection(RecordCollection *decoderResult, unsigned int recordNumber,
    ...
    for (unsigned int i = 0; i < numberOfItems && decodedElementNames->size() > 0; i++) {
        ...
        if (itemToEncodeDescription->getName() != "-" && itemToEncodeDescription->isItemToBeEncoded()) {
            ...
            switch (itemType) {
                ...
                case ItemTypeEnum::FixedLength:
                    itemBytestream = ItemEncodingUtils::encodeFixedLengthItem(decodedElementNames, decodedElementValues,
                                                                              itemToEncodeDescription,
                                                                              categoryNumber);
                    break;

                case ItemTypeEnum::ExtendedLength:
                    itemBytestream = ItemEncodingUtils::encodeExtendedLengthItem(decodedElementNames,
                                                                                 decodedElementValues,
                                                                                 itemToEncodeDescription,
                                                                                 categoryNumber);
                    break;

                case ItemTypeEnum::RepetitiveLength:
                    itemBytestream = ItemEncodingUtils::encodeRepetitiveLengthItem(decodedElementNames,
                                                                                   decodedElementValues,
                                                                                   itemToEncodeDescription,
                                                                                   categoryNumber);
                    break;
                ...
                case ItemTypeEnum::CompoundLength:
                    itemBytestream = ItemEncodingUtils::encodeCompoundLengthItem(decodedElementNames,
                                                                                 decodedElementValues,
                                                                                 itemToEncodeDescription,
                ...
                case ItemTypeEnum::ExplicitLength:
                    itemBytestream = ItemEncodingUtils::encodeExplicitLengthItem(decodedElementNames,
                                                                                 decodedElementValues,
                                                                                 itemToEncodeDescription,
                                                                                 compoundSubItemsDescription,
                                                                                 refItemDescription, spfItemDescription,
                                                                                 categoryNumber);
                    break;
                case ItemTypeEnum::VariableLength:
                {
                    ElementValue *encodingElementValue = nullptr;
                    encodingElementValue = decodedElementValues->at(1);
                    uint16_t varsize = ((UnsignedInteger16bitValue*)encodingElementValue)->getDecodedValue();
                    itemToEncodeDescription->setLengthInByte(varsize+2);
                    vector<int> ElementSize;
                    ElementSize.push_back(16);
                    ElementSize.push_back(varsize*8);
                    itemToEncodeDescription->setAllElementDescription(ElementSize);
                    itemBytestream = ItemEncodingUtils::encodeFixedLengthItem(decodedElementNames, decodedElementValues,
                                                                              itemToEncodeDescription,
                                                                              categoryNumber);
                }
        ...
        if (itemBytestream != nullptr && itemBytestream->size() != 0) {
            ...
            if (itemToEncodeDescription->isItemMandatory() == true) {
                encoderFailureReport->addFailure(categoryNumber, 0, itemToEncodeDescription->getName(), nullptr,
                                                 "A Mandatory Item for the current Category was missing!",
                                                 ReturnCodes::MISSING_MANDATORY_ITEM);
            }
        }
    ...
    if (CommonVariables::getInstance()->isEncodingDebugEnabled()) {
        ...
        for (unsigned int i = 0; i < FSPEC_lengthInByte; i++) {
            cout << " " << std::setw(2) << std::setfill('0') << std::hex
                 << static_cast<unsigned int>(recordBytestream->at(i));
        }
        cout << endl;
    }
 ...
 */
vector<unsigned char> *
encodeAllItemsFromSingleRecord(Record *recordToEncode, CategoryDescription *categoryDescription,
    ...
    for (map<string, ElementValue *>::iterator element_it = recordToEncode->getElements()->begin();
         element_it != recordToEncode->getElements()->end();
         element_it++) {
    ...
    for (unsigned int i = 0; i < numberOfItems && elementNames->size() > 0; i++) {
        ...
        if (itemToEncodeDescription->getName() != "-" && itemToEncodeDescription->isItemToBeEncoded()) {
            ...
            switch (itemToEncodeDescription->getItemType()) {
                case ItemTypeEnum::FixedLength:
                    itemBytestream = ItemEncodingUtils::encodeFixedLengthItem(elementNames, elementValues,
                                                                              itemToEncodeDescription, categoryNumber);
                    break;

                case ItemTypeEnum::ExtendedLength:
                    itemBytestream = ItemEncodingUtils::encodeExtendedLengthItem(elementNames, elementValues,
                                                                                 itemToEncodeDescription,
                                                                                 categoryNumber);
                    break;

                case ItemTypeEnum::RepetitiveLength:
                    itemBytestream = ItemEncodingUtils::encodeRepetitiveLengthItem(elementNames, elementValues,
                                                                                   itemToEncodeDescription,
                                                                                   categoryNumber);
                    break;

                case ItemTypeEnum::CompoundLength:
                    itemBytestream = ItemEncodingUtils::encodeCompoundLengthItem(elementNames, elementValues,
                                                                                 itemToEncodeDescription,
                                                                                 compoundSubItemsDescription,
                                                                                 categoryNumber);
                    break;
                ...
                case ItemTypeEnum::ExplicitLength:
                    itemBytestream = ItemEncodingUtils::encodeExplicitLengthItem(elementNames, elementValues,
                                                                                 itemToEncodeDescription,
                                                                                 compoundSubItemsDescription,
                                                                                 refItemDescription, spfItemDescription,
                                                                                 categoryNumber);
                ...
                {
                    ElementValue *encodingElementValue = nullptr;
                    encodingElementValue = elementValues->at(1);
                    vector<int> *itemLength = itemToEncodeDescription->getAllElementDescription();
                    vector<int> ElementSize;
                    int varsize;
                    switch (itemLength->at(0)) {
                        case 8:
                            varsize = ((UnsignedInteger8bitValue*)encodingElementValue)->getDecodedValue();
                            itemToEncodeDescription->setLengthInByte(varsize + 1);
                            ElementSize.push_back(8);
                            ElementSize.push_back(varsize*8);
                            itemToEncodeDescription->setAllElementDescription(ElementSize);
                            break;
                        case 16:
                            varsize = ((UnsignedInteger16bitValue*)encodingElementValue)->getDecodedValue();
//                            varsize = std::stoi(elementValues->at(1)->getDecodedDataInString());
                            itemToEncodeDescription->setLengthInByte(varsize + 1);
                            ElementSize.push_back(16);
                            ElementSize.push_back(varsize*8);
                            itemToEncodeDescription->setAllElementDescription(ElementSize);
...
unsigned char *encodeForDecodedRecord(RecordCollection *recordsToEncodeCollection, unsigned int recordIndex) {
    ...
    if (categoryDescription_it == categoriesDescription->end()) {
        encoderFailureReport->addFailure(categoryNumber, 0, "", nullptr, "Unknown Category number!",
     ...
     * START ENCODING *
     ******************/
    recordBytestream = encodeAllItemsFromRecordCollection(recordsToEncodeCollection, recordIndex, categoryDescription,
                                                          categoryNumber);
...
unsigned char *encodeForSingleRecord(Record *recordToEncode) {
    ...
    if ((elementsMap == nullptr) || (elementsMap->size() == 0)) {
        encoderFailureReport->addFailure(categoryNumber, 0, "", nullptr, "Record to be encoded is empty!",
                                         ReturnCodes::ENCODING_ERROR);
        return nullptr;
    }
