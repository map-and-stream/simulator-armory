...
unsigned char *EncoderInterface::encodeRecords(RecordCollection *recordsToEncode, unsigned int *recordNumberEncoded) {
    ...
    if (isAstOutputEnable) {
        outFile.open(common->getOutputAstPathForEncoder(),
                     ios::out | ios::binary | ios::app);    // "ios::app" to append at the end of the file
    }
    ...
    for (auto &&result : encoderResults) {
        ...
        if (isEncodingDebugEnabled) {
            ...
            cout << std::dec << "CAT: " << static_cast<int>(datablockCategoryEncodedRecord) << " - LEN: "
                 << static_cast<int>(datablockLengthEncodeRecord)
                 << endl;
            cout << endl << endl;
        }
    ...
    if (isAstOutputEnable) {
        ...
        for (unsigned int i = 0; i < encodedDatablocksCollection.size(); i++) {
            datablockLengthEncodeRecord = (unsigned int) (encodedDatablocksCollection.at(i)[2] +
                                                          ((encodedDatablocksCollection.at(i)[1] << 8) & 0xFF00));
...
unsigned char *EncoderInterface::encodeSingleRecord(Record *recordToEncode, unsigned int *recordNumberEncoded) {
    ...
    for (auto &&result : encoderResults) {
        ...
        if (common->isEncodingDebugEnabled()) {
            ...
            cout << endl << "*** DATABLOCK ***" << endl;
            cout << std::dec << "CAT: " << static_cast<int>(encodedCategoryNumber) << " - LEN: "
                 << static_cast<int>(encodedDatablockLength) << endl;
            cout << endl << endl;
        }
    ...
    // Build a complete bytestream returned to the client
    outputRawData = new unsigned char[totalBytestreamSize + 5]();
    int offset = 0;

    // Each raw data collected is copied in the output bytestream
    for (unsigned int i = 0; i < encodedDatablocksCollection.size(); i++) {
        encodedDatablockLength = (unsigned int) (encodedDatablocksCollection.at(i)[2] +
                                                 ((encodedDatablocksCollection.at(i)[1] << 8) & 0xFF00));
...
//    unsigned int  leng = (unsigned int ) totalBytestreamSize;
//    processor.addHeader(outputRawData, &leng);

    const unsigned char header[] = {0x6e,0x6e,0x6e,0x89,outputRawData[1],outputRawData[2],outputRawData[0],0x00};
    const unsigned int headerLength = 8;
    std::memmove(outputRawData + headerLength, outputRawData + 3, totalBytestreamSize - 3);
    std::memcpy(outputRawData, header, headerLength);
    totalBytestreamSize = totalBytestreamSize + 5;

//    totalBytestreamSize = leng;

    return outputRawData;
}

void EncoderInterface::printASTOnFile(unsigned char *civil_bytestream) {
    int datablockLength = (unsigned int) (civil_bytestream[2] + ((civil_bytestream[1] << 8) & 0xFF00));
    outFile.write((char *) civil_bytestream, datablockLength);
}
