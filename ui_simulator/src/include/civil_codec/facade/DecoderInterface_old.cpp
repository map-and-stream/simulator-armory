...
void DecoderInterface::setSourceDatastream(unsigned char *_civil_datastream, unsigned long _ASTinput_fileSize) {
    civil_datastream = _civil_datastream;
    ASTinput_fileSize = _ASTinput_fileSize;
}

RecordCollection *
DecoderInterface::startDecodingDatablocks(unsigned char *civil_datastream, unsigned long *endOfBufferDecodingIndex) {
    ...
    unsigned long bufferIndex = 0;                // Main index to move around the 'civil_datastream' buffer
    ...
    while (bufferIndex < (*endOfBufferDecodingIndex)) {
        ...
        datablockCategory = (unsigned int) civil_datastream[bufferIndex];
        // Reading the datablock Length value (2 Bytes)
        datablockLength = (unsigned int) (civil_datastream[bufferIndex + 2] +
...
RecordCollection *DecoderInterface::decodingDatablocksFromFile() {
    ...
    unsigned long startIndexOfDatablocks = indexIncivil_datastream;
    ...
    while ((numberOfDatablocksToParse != currentNumbeOfDatablocks) && (indexIncivil_datastream < ASTinput_fileSize)) {

        // Reading the datablock Length value (2 Bytes)
        datablockLength = (unsigned int) (civil_datastream[indexIncivil_datastream + 2]
                                          + ((civil_datastream[indexIncivil_datastream + 1] << 8) &
                                             0xFF00));    //1111 1111 0000 0000b

        if ((indexIncivil_datastream + datablockLength) <= ASTinput_fileSize) {
            currentNumbeOfDatablocks++;
            indexIncivil_datastream += datablockLength;
        } else {
            // Last Datablock is not complete
            indexIncivil_datastream = ASTinput_fileSize;    // Set the current index at the end of the file to Stop the outer loop, end of file reached
        }
    }
    ...
    lengthOfBytestreamToParse = indexIncivil_datastream - startIndexOfDatablocks;
    return startDecodingDatablocks(civil_datastream + startIndexOfDatablocks, &lengthOfBytestreamToParse);
}
...
void DecoderInterface::printAllDecodedRecordsOnJson(RecordCollection *recordsToPrint) {
    ...
    for (unsigned int i = 0; i < recordsToPrintSize; i++) {
        ...
        sizeOfElementsToPrint = elementValues_toPrint->size();
        ...
        for (unsigned int j = 0; j < sizeOfElementsToPrint; j++) {
            ...
            jsonElementValue.SetString(elementValue->getDecodedDataInString().c_str(),
                                       elementValue->getDecodedDataInString().length(),
                                       allocator);
