...
void DecoderInterface::setSourceDatastream(unsigned char *_civil_datastream, unsigned long _ASTinput_fileSize) {
    civil_bytestream = _civil_datastream;
    inputFile_sizeInByte = _ASTinput_fileSize;
}

RecordCollection *
DecoderInterface::startDecodingDatablocks(unsigned char *civil_datastream, unsigned int *lengthOfBytestreamToParse) {
    ...
    unsigned long bufferIndex = 0;                  // Main index to move around the 'civil_bytestream' buffer
    unsigned long errorIndex = 0;


    if (lengthOfBytestreamToParse[0] < 10 || civil_datastream[0] != 0x6e || civil_datastream[1] != 0x6e || civil_datastream[2] != 0x6e || civil_datastream[3] != 0x89) {
        string message = "The Datablock provided not starting with 0x6e6e6e89" ;
        FailureReport::getInstance()->addFailure(datablockCategory, bufferIndex, "", nullptr, message,
                                                 ReturnCodes::PARSING_ERROR);
        return decodedRecords;
    }

    const unsigned char header[] = {civil_datastream[6],civil_datastream[4],civil_datastream[5]};
    const unsigned int headerLength = 3;
    std::memcpy(civil_datastream, header, headerLength);
    std::memmove(civil_datastream + 3, civil_datastream + 8, (*lengthOfBytestreamToParse) - 8);
    lengthOfBytestreamToParse[0] = lengthOfBytestreamToParse[0] - 5;
    ...
    // There are bytes to read while 'bufferIndex' is less than 'civil_inputFileSize'
    while (bufferIndex < (*lengthOfBytestreamToParse)) {

        // Reading the datablock Category number (1 Byte)
        datablockCategory = (unsigned short int) civil_datastream[bufferIndex];
        // Reading the datablock Length value (2 Bytes)
        datablockLength = (unsigned short int) (civil_datastream[bufferIndex + 2] +
                ((civil_datastream[bufferIndex + 1] << 8) & 0xFF00));

        if ((bufferIndex + datablockLength) <= (*lengthOfBytestreamToParse)) {
            parsingResults.push_back(
                        parserTP->enqueue(ParsingUtils::parseDatablock, civil_datastream, datablockCategory,
            ...
            string message = "The Datablock provided starting at index " + to_string(bufferIndex) +
                    ...
                    " position.";
            FailureReport::getInstance()->addFailure(datablockCategory, errorIndex, "", civil_datastream, message,
                                                     ReturnCodes::PARSING_ERROR);
            break;
        }
        errorIndex += (datablockLength + 5);

        //edit javad
//        bufferIndex += datablockLength;

        if (bufferIndex < (*lengthOfBytestreamToParse) && datablockLength < lengthOfBytestreamToParse[0]) {
            civil_datastream = civil_datastream +datablockLength;
            lengthOfBytestreamToParse[0] = lengthOfBytestreamToParse[0] - datablockLength;
//            processor.removeHeader(civil_datastream, lengthOfBytestreamToParse);
            const unsigned char header[] = {civil_datastream[6],civil_datastream[4],civil_datastream[5]};
            const unsigned int headerLength = 3;
            std::memcpy(civil_datastream, header, headerLength);
            std::memmove(civil_datastream + 3, civil_datastream + 8, (*lengthOfBytestreamToParse) - 8);
...
unsigned int DecoderInterface::setupDecodingFromFile() {
    ...
    unsigned int civilBytestream_index_start = civilBytestream_index;
    unsigned short int datablock_lengthInByte = 0;            // Datablock LEN field value
    unsigned int civilBytestream_toParse_lengthInByte = 0;

    // There are bytes to read until 'civilBytestream_index' is less than 'inputFile_sizeInByte'
    //if currentNumberOfDatablocks == numberOfDatablocksToParse -> we have collect enough datablocks -> we can decode
    while ((civilBytestream_index < inputFile_sizeInByte) &&
           (currentNumberOfDatablocks < numberOfDatablocksToParse)) {
        // Reading the datablock Length value (2 Bytes)
        datablock_lengthInByte = (unsigned short int) (civil_bytestream[civilBytestream_index + 2]
                + ((civil_bytestream[civilBytestream_index + 1] << 8) &
                0xFF00));    //1111 1111 0000 0000b

        if ((civilBytestream_index + datablock_lengthInByte) <= inputFile_sizeInByte) {
            currentNumberOfDatablocks++;
            civilBytestream_index += datablock_lengthInByte;
        } else {
            // Last Datablock is not complete -> it will be dropped
            civilBytestream_index = inputFile_sizeInByte;
            // The 'civilBytestream_index' is placed at the end of the file to stop the outer loop: EOF reached.
        }
    }
    ...
    civilBytestream_toParse_lengthInByte = civilBytestream_index - civilBytestream_index_start;
    return civilBytestream_toParse_lengthInByte;
}

RecordCollection *DecoderInterface::decodeFromFile() {
    unsigned int civilBytestream_initial_offset = civilBytestream_index;
    unsigned int civilBytestream_toParse_lengthInByte = setupDecodingFromFile();

    return startDecodingDatablocks(civil_bytestream + civilBytestream_initial_offset,
                                   &civilBytestream_toParse_lengthInByte);
}
