...
//** OTHER FUNCTIONS **
void CodecInterface::loadcivilConfiguration(std::string pathTocivilConfigurationFile, ReturnStatus &status) {
    status.clear();

    unordered_map<int, CategoryDescription *> *categoriesDescription = common->getCategoriesDescription();

    if (pathTocivilConfigurationFile.find(".dat") != std::string::npos) {
//        JsonCategoriesDescriptionLoader::getCategoriesDescriptionMapFromSerialization(pathTocivilConfigurationFile,
//                                                                                      categoriesDescription, &status);
    } else {
        JsonCategoriesDescriptionLoader::loadcivilDescription(pathTocivilConfigurationFile, categoriesDescription,
                                                                &status);
    }
}

void CodecInterface::serializecivilConfiguration(std::string pathToArchiveFile, ReturnStatus &status) {
    status.clear();
