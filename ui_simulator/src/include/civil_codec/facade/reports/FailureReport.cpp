...
FailureReport *FailureReport::getInstance() {
    if (failureReportSingleton == nullptr) {
        std::unique_lock<std::mutex> ul(
                instantiationMutex);    // Get the lock to instantiate for the first time the Report
        failureReportSingleton = new FailureReport();
    }
    return failureReportSingleton;
}

void FailureReport::addFailure(unsigned int categoryNumber, unsigned int civil_bytestreamOffset, std::string itemName,
    ...
    civil_bytestreamOffsets.push_back(civil_bytestreamOffset);
...
Failure FailureReport::get(unsigned int index) {
    ...
    if (categoryNumbers.size() > index) {
        ...
        failure.bytestreamOffset = civil_bytestreamOffsets[index];
        ...
        if (rawDatablocks[i] != nullptr) {
            ...
            for (unsigned int j = 0; j < datablockLength; j++) {
                if (rawDatablocks[i][j]) {
                    outFile << std::hex << std::setw(1) << std::setfill('0')
                            << static_cast<unsigned int>(rawDatablocks[i][j]) << "";
                }
            }
            outFile << endl << endl;
        }
