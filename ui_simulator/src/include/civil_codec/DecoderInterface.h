...
public:
     ...
     * @param _civil_datastream Pointer to the civil bytestream opened from the client
     * @param _ASTinput_fileSize Size in byte of the input civil file
     */
    void setSourceDatastream(unsigned char *_civil_datastream, unsigned long _ASTinput_fileSize);
    ...
    /**
     * TODO
     * DECODE - Parses the civil bytestream already loaded into memory.
     * @param civil_datastream Pointer to the civil bytestream into memory
     * @param lengthOfBytestreamToParse Size in byte of the input file stream
     * @return Collection of decoded records
     */
    RecordCollection *
    startDecodingDatablocks(unsigned char *civil_datastream, unsigned int *lengthOfBytestreamToParse);
...
private:
    ...
    /**
     * Variable to keep in memory the civil bytestream
     */
    unsigned char *civil_bytestream;

    /**
     * Variable to keep in memory the size in byte of the 'civil_datastream'
     */
    unsigned int inputFile_sizeInByte;

    /**
     * Main index to move around the 'civil_datastream' buffer
     */
    unsigned int civilBytestream_index = 0;
